...
#Why did we make it that way? You can try running your code locally so that the payment can be any dollar 
...
#that we can pay off the entire balance within a year. What is a reasonable lower bound for this payment value? $0 is the obvious anwer, but you can do better than that. If there was no interest, the debt can be paid off by monthly payments of one-twelfth of the original balance, so we must pay at least this much every month. One-twelfth of the original balance is a good lower bound.
#
...
# than what we would've paid in monthly installments, because the interest was compounded on the 
# balance we didn't pay off each month.
...
#Wikipedia page on bisection search) to find the smallest monthly payment to the cent (no more multiples of $10) 
...
upperbound = (newbalance * (1 + monthlyInterestRate)**12)/12.0
epsilon = 0.01
while abs(balance) > epsilon:
    minPay = (upperbound+lowerbound)/2
    balance = newbalance
    for i in range(12):
        balance = balance-minPay +((balance-minPay)*monthlyInterestRate)
    if balance > epsilon:
        lowerbound = minPay
